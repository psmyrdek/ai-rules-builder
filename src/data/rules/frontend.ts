...
export const frontendRules: LibraryRulesMap = {
  ...
  [Library.REACT_ROUTER]: [
    ...
    'Implement error boundaries with errorElement to gracefully handle routing and data loading errors',
  ...
  [Library.REDUX]: [
    ...
    'Implement middleware selectively and avoid overusing thunks for simple synchronous operations',
  ],
  [Library.ZUSTAND]: [
    ...
    'Leverage the persist middleware for automatic state persistence in localStorage or sessionStorage',
  ...
  [Library.REACT_QUERY]: [
    ...
    'Leverage queryClient.setQueryDefaults to establish consistent settings for query types',
    'Use suspense mode with <Suspense> boundaries for a more declarative data fetching approach',
    'Implement retry logic with custom backoff algorithms for transient network failures',
  ],
  ...
  [Library.VUEX]: [
    ...
    'Leverage plugins for cross-cutting concerns like persistence, logging, or analytics',
  ...
  [Library.VUE_ROUTER]: [
    ...
    'Leverage route meta fields to store additional information about routes for custom logic',
  ...
  [Library.PINIA]: [
    ...
    'Use plugins for cross-cutting concerns like persistence, state reset, or devtools customization',
  ...
  [Library.NGRX]: [
    ...
    'Implement @ngrx/effects for handling side effects and asynchronous operations',
  ...
  [Library.ANGULAR_MATERIAL]: [
    ...
    'Implement accessibility attributes and ARIA labels for all interactive components',
  ],
  ...
  [Library.SVELTE_KIT]: [
    ...
    'Leverage SvelteKit\'s server-only modules for sensitive operations that shouldn\'t run on the client',
    'Implement +error.svelte files for custom error handling at the route level',
    'Use the enhance function for progressive enhancement of form submissions',
  ],
  ...
  [Library.ASTRO_ISLANDS]: [
    ...
    'Use Astro.cookies API for server-side cookie management instead of client-side solutions',
  ],
};
