...
export const getLayerClasses = {
  container: (layerType: LayerType, isSelected: boolean): string => {
    return isSelected
      ...
      : `bg-gray-800/70 hover:bg-gray-700/70 border-${layerColors[layerType]}-100 border hover:shadow-sm`;
  },
  
  openState: (layerType: LayerType, isOpen: boolean): string => {
    return isOpen
      ? `mb-1 ring-1 ring-${layerColors[layerType]}-300`
      : '';
  },
  
  badge: (layerType: LayerType): string => {
    return `bg-${layerColors[layerType]}-500 text-white`;
  },
  ...
  stackContainer: (layerType: LayerType, isSelected: boolean): string => {
    return isSelected
      ...
      : `bg-gray-700/60 hover:bg-gray-600/60 border-${layerColors[layerType]}-200 border hover:shadow-sm`;
  },
  ...
  toggleSwitch: (layerType: LayerType, isSelected: boolean): { borderColor: string, backgroundColor: string } => {
    return {
      borderColor: isSelected ? `var(--tw-${layerColors[layerType]}-500)` : 'transparent',
  ...
  toggleHandle: (layerType: LayerType, isSelected: boolean): { backgroundColor: string, transform: string } => {
    return {
      backgroundColor: isSelected ? `var(--tw-${layerColors[layerType]}-500)` : '#D1D5DB',
...
export const layerToType = (layer: Layer): LayerType => {
  switch (layer) {
    case Layer.CRAFTSMANSHIP: return 'craftsmanship';
    case Layer.FRONTEND: return 'frontend';
    case Layer.BACKEND: return 'backend';
    case Layer.DATABASE: return 'database';
    case Layer.DEVOPS: return 'devops';
    case Layer.TESTING: return 'testing';
    default: return 'craftsmanship';
  }
};
